...
`include "av1_tx4x4_chroma_tables.vh"
    ...
    // The 64x64, width-stretched <=64-row, 64x128, 128x128, and
    // 192x128, 256x128, and 320x128 full-coeff reduced reference-stack/MV-prediction
    // paths are now public-clean for the natural unrestricted LAST-frame stress cases. Keep
    // the conservative cap on other taller geometries until their unrestricted
    // stack model is decoder-exact. The disable_fullcoeff_cap_in hook remains
    // testbench-only for larger-size boundary probes. DC-only tests retain
    // their uncapped fractional/NEAREST/NEARMV syntax ownership coverage.
    wire        me_auto_zero_mv = (me_newmv_limit_in == 8'd0) && !dc_only_in;
    wire        me_integer_mv_only = !dc_only_in;
    wire        me_fullcoeff_tall_public_clean =
        ((FRAME_WIDTH == 64) && (FRAME_HEIGHT <= 128)) ||
        ((FRAME_WIDTH == 128) && (FRAME_HEIGHT == 128)) ||
        ((FRAME_WIDTH == 192) && (FRAME_HEIGHT == 128)) ||
