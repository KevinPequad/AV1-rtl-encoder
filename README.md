...
- Implemented:
  ...
  - the P7 reference boundary checkpoint is now documented in `P7_REFERENCE_BOUNDARY.md` and validated on canonical `main` at `8994490d209a745fb157e999af616b49de2c6ce1` (already pushed to `origin/main`); see that doc for the exact 21/21 gate split, q3/mode-context evidence, and compound/reference deferrals; compound / multi-ref tools stay disabled for this checkpoint while the full RTL AV1 roadmap remains open
- Validated:
  ...
  - the single-threaded P7 current matrix on Chud PC 2 passed 21/21 gates: T8 stayed a syntax/byte-ownership gate with `frame=1 total_inter=64 nonzero_inter=3 first_inter_blk=0 mode_counts={GLOBALMV:45 NEARESTMV:1 NEARMV:2 NEWMV:16}`, while T9 exercised the fractional q3 NEWMV set `[(0, 0, -4), (1, -4, 64)]` and matched FFmpeg/libdav1d plus `aomdec`
