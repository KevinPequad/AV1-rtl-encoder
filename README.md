...
- The strict raw-path inter checkpoints are now cleared on both the zero-motion repeated-frame cases and the first natural-motion cases:
  ...
  - the unrestricted 3-frame widening probe remains byte-exact between RTL and the software oracle, but frame 2 still has the narrow public-decoder Cb +1 delta at blk33/34; the probe now pins the source-backed unscaled chroma origin to base `(11,11)` / phase `(8,0)` in both AV1 spec math and current libaom `reconinter.h`, so the next fix should be based on decoder-side MV/ref-sample evidence rather than a blind chroma phase offset
