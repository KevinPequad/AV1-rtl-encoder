...
- The current RTL raw keyframe stream still diverges from the software-owned valid stream at the frame OBU size / payload ownership boundary: the header fields now line up, but final frame sizing and tile payload packaging are not RTL-owned yet.
