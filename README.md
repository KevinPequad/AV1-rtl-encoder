...
- The strict raw-path inter checkpoints are now cleared on both the zero-motion repeated-frame cases and the first natural-motion cases:
  ...
  - the unrestricted 3-frame widening probe remains byte-exact between RTL and the software oracle, but frame 2 still has the narrow public-decoder Cb +1 delta at blk33/34; the probe now pins the source-backed unscaled chroma origin to base `(11,11)` / phase `(8,0)` in both AV1 spec math and current libaom `reconinter.h`, and also verifies FFmpeg/libdav1d plus `aomdec` only differ from RTL at local Cb sample `(1,3)` in each 4x4 block while blk34 keeps RTL's same +4 DC residual, so the next fix should be based on decoder-side MV/ref-sample evidence rather than a blind chroma phase offset or residual-path patch
