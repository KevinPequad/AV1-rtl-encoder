...
- The strict raw-path inter checkpoints are now cleared on both the zero-motion repeated-frame cases and the first natural-motion cases:
  ...
  - the `64x64` full-coeff low-delay LAST gradient gate is currently public-decoder clean through the first `37` requested integer `NEWMV` blocks under the reduced cap: RTL raw OBU and IVF match the software oracle, and FFmpeg/libdav1d plus `aomdec` decode back to `recon.yuv`
