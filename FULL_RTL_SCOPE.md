...
The active backlog is full feature-complete RTL AV1. The reduced pre-ASIC freeze is a historical checkpoint only, and the freeze docs must never be read as the endpoint of the project. Use this matrix as the active source of truth; use the historical freeze docs only as evidence for the reduced checkpoint.
