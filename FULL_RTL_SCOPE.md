...
| P11 | Input/output formats, resolution scaling, final target | Inventory 2 and 9 | 8-bit 4:2:0 raw YUV plus small fixtures exist | Big Buck Bunny `1280x720 @ 24 fps` representative clip from the RTL byte path, plus packaging / metadata | The smallest full-resolution low-delay clip that proves meaningful inter/reference behavior must decode publicly and match/track RTL recon; 2 frames is acceptable for KEY→INTER proof, and 240 frames is optional soak evidence, not a required proof gate |
