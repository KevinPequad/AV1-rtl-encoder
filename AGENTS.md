...
- The first real non-zero fractional-pel translational checkpoint on the reduced LAST-only path is now exact on the named natural32/natural64 fixtures; the software writer/testbench still serves as the oracle for those comparisons.
- The `64x64` full-coeff low-delay LAST gradient checkpoint is public-decoder clean through the first `40` requested non-zero integer motion blocks under the reduced cap (`39` `NEWMV` payload blocks plus one stack-hit `NEARESTMV` block): RTL raw OBU and IVF match the software oracle, and FFmpeg/libdav1d plus `aomdec` match `recon.yuv`. Keep widening by making the unrestricted reference-stack/MV-prediction model exact; do not report cap raises as full completion.
- The current unrestricted 3-frame full-coeff NEWMV blocker is narrowed but not fixed: RTL/software bytes match, FFmpeg/libdav1d and `aomdec` agree, and the only decoder/recon delta is frame-2 Cb +1 at blk33/34. The probe now proves AV1 spec math and current libaom unscaled chroma setup both keep those samples at base `(11,11)` / phase `(8,0)`; do not mask the blocker with a blind `av1_chroma_inter_pred` phase offset without decoder-side MV/ref-sample evidence.
- The next ownership move is widening beyond the current reduced single-reference LAST syntax-only subpel subset into broader natural-motion clips and multi-reference/reference-MV-context debugging:
  ...
  - then continue the reduced inter roadmap in this order:
    - widen beyond the reduced single-reference LAST motion subset into multi-reference/reference-MV-context natural clips
