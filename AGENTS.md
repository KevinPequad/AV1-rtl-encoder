...
- The `64x64` full-coeff low-delay LAST gradient checkpoint is public-decoder clean through the first `37` requested integer `NEWMV` blocks under the reduced cap: RTL raw OBU and IVF match the software oracle, and FFmpeg/libdav1d plus `aomdec` match `recon.yuv`. Keep widening by making the unrestricted reference-stack/MV-prediction model exact; do not report cap raises as full completion.
