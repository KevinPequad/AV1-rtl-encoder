...
- The `64x64` full-coeff low-delay LAST gradient checkpoint is public-decoder clean unrestricted, and the 128-high reduced reference-stack frontier is now public-clean through `1920x128` without the full-coeff cap (`GLOBALMV=3585`, `NEARESTMV=155`, `NEWMV=100` on the 1920x128 probe): RTL raw OBU and IVF match the software oracle, and FFmpeg/libdav1d plus `aomdec` match `recon.yuv`. Keep widening by making the unrestricted reference-stack/MV-prediction model exact; do not report cap raises as full completion.
