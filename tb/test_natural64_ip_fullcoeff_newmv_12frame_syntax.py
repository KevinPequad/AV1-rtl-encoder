#!/usr/bin/env python3
"""12-frame 64x64 full-coeff low-delay LAST unrestricted NEWMV proof.

This extends the strict full-coeff NEWMV public-decoder/recon parity checkpoint
past the 11-frame LAST-chain gate.  The reduced reference-stack/MV-prediction path
stays public-decoder exact through one more LAST refresh without raising the
NEWMV cap: frame 11 settles to a 10 NEWMV / 1 NEARESTMV mix while RTL and
...
def main() -> int:
    ...
    out = artifact_root / "natural64_ip_fullcoeff_newmv_12frame"
    ...
    check_public_decoder_case(paths, "12-frame 64x64 full-coeff unrestricted NEWMV LAST-chain")
    ...
    for label, path in (
        ...
        if got != expected_md5s:
            fail(f"{label} 12-frame MD5 scope drifted: {got}")

    print(
        "[PASS] 12-frame 64x64 full-coeff unrestricted NEWMV LAST-chain: "
        "RTL/software bytes match, FFmpeg/libdav1d and aomdec decode cleanly, "
        "decoder output is byte-identical to RTL recon, and frame-11 stays at "
        "GLOBALMV=53 NEARESTMV=1 NEARMV=0 NEWMV=10"
    )
    return 0
