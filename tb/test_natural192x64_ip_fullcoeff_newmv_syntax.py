#!/usr/bin/env python3
"""192x64 full-coeff low-delay LAST inter proof with unrestricted integer motion.

This is the next width-stretched geometry beyond the 128x64 full-coeff
...
public-decoder exact for the deterministic 192x64 gradient stress path.
"""
...
def main() -> int:
    if not SIM.exists():
        fail(f"missing simulator {SIM}; run make WIDTH=192 HEIGHT=64 all first")
    artifact_root = Path(os.environ.get("AV1_ARTIFACT_ROOT", TB / "artifacts"))
    out = artifact_root / "natural192x64_ip_fullcoeff_newmv"
    paths = run_encoder_case(
        ...
        dc_only=0,
        timeout=800_000_000,
        # Aggregate-only inter summary keeps this width-stretched unrestricted
        # NEWMV proof cron-safe while still pinning the reduced-stack mix.
    ...
    if globalmv != 114 or newmv != 56 or nearestmv != 22 or nearmv != 0:
        fail(
            f"expected width-stretched unrestricted mix GLOBALMV=114 NEWMV=56 NEARESTMV=22 NEARMV=0, "
            f"saw global={globalmv} new={newmv} nearest={nearestmv} near={nearmv}"
        )
    print(
        "[PASS] 192x64 full-coeff unrestricted integer motion summary: "
    ...
    check_public_decoder_case(paths, "192x64 full-coeff unrestricted integer motion")
    print("[PASS] 192x64 full-coeff unrestricted integer motion public-decoder proof")
    return 0
