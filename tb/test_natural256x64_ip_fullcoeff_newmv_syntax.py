#!/usr/bin/env python3
"""256x64 full-coeff low-delay LAST inter proof with unrestricted integer motion.

This extends the width-stretched 64-high NEWMV/reference-stack public-clean
lane beyond 192x64.  It proves the deterministic 256x64 gradient stress path is
RTL/software byte-identical and decoder-to-recon exact without relaxing the
taller-than-64 cap.
"""
...
def main() -> int:
    if not SIM.exists():
        fail(f"missing simulator {SIM}; run make WIDTH=256 HEIGHT=64 all first")
    artifact_root = Path(os.environ.get("AV1_ARTIFACT_ROOT", TB / "artifacts"))
    out = artifact_root / "natural256x64_ip_fullcoeff_newmv"
    ...
    if globalmv != 162 or newmv != 60 or nearestmv != 34 or nearmv != 0:
        fail(
            f"expected width-stretched unrestricted mix GLOBALMV=162 NEWMV=60 NEARESTMV=34 NEARMV=0, "
            f"saw global={globalmv} new={newmv} nearest={nearestmv} near={nearmv}"
        )
    print(
        "[PASS] 256x64 full-coeff unrestricted integer motion summary: "
    ...
    check_public_decoder_case(paths, "256x64 full-coeff unrestricted integer motion")
    print("[PASS] 256x64 full-coeff unrestricted integer motion public-decoder proof")
    return 0
