#!/usr/bin/env python3
"""7-frame 64x64 full-coeff low-delay LAST unrestricted NEWMV proof.

This extends the strict full-coeff NEWMV public-decoder/recon parity checkpoint
past the 6-frame LAST-chain gate.  The reduced reference-stack/MV-prediction path
stays public-decoder exact through one more LAST refresh without raising the
NEWMV cap: frame 6 settles to the current 14 NEWMV / 0 NEARESTMV mix while RTL
and software bytes remain identical.
"""
...
def main() -> int:
    ...
    out = artifact_root / "natural64_ip_fullcoeff_newmv_7frame"
    ...
    check_public_decoder_case(paths, "7-frame 64x64 full-coeff unrestricted NEWMV LAST-chain")
    ...
    for label, path in (
        ...
        if got != expected_md5s:
            fail(f"{label} 7-frame MD5 scope drifted: {got}")

    print(
        "[PASS] 7-frame 64x64 full-coeff unrestricted NEWMV LAST-chain: "
        "RTL/software bytes match, FFmpeg/libdav1d and aomdec decode cleanly, "
        "decoder output is byte-identical to RTL recon, and frame-6 stays at "
        "GLOBALMV=50 NEARESTMV=0 NEARMV=0 NEWMV=14"
    )
    return 0
