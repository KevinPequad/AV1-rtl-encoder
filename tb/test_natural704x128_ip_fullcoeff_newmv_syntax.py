#!/usr/bin/env python3
"""704x128 unrestricted full-coeff low-delay LAST inter proof.

This graduates the next wider 128-high full-coeff NEWMV/reference-stack
checkpoint beyond 640x128. It proves the deterministic 704x128 gradient stress
...
def main() -> int:
    if not SIM.exists():
        fail(f"missing simulator {SIM}; run make WIDTH=704 HEIGHT=128 all first")
    artifact_root = Path(os.environ.get("AV1_ARTIFACT_ROOT", TB / "artifacts"))
    out = artifact_root / "natural704x128_ip_fullcoeff_newmv"
    ...
    if globalmv != 1364 or newmv != 42 or nearestmv != 2 or nearmv != 0:
        fail(
            f"expected unrestricted taller-geometry mix GLOBALMV=1364 NEWMV=42 NEARESTMV=2 NEARMV=0, "
            f"saw global={globalmv} new={newmv} nearest={nearestmv} near={nearmv}"
        )
    print(
        "[PASS] 704x128 unrestricted full-coeff integer motion summary: "
    ...
    check_public_decoder_case(paths, "704x128 unrestricted full-coeff integer motion")
    print("[PASS] 704x128 unrestricted full-coeff integer motion public-decoder proof")
    return 0
