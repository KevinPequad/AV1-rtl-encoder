#!/usr/bin/env python3
"""1792x64 full-coeff low-delay LAST inter proof with unrestricted integer motion.

This extends the width-stretched 64-high NEWMV/reference-stack public-clean
lane to 1792x64. It keeps the taller-than-64 cap intact while proving the
deterministic 1792x64 gradient stress path is RTL/software byte-identical and
...
def main() -> int:
    if not SIM.exists():
        fail(f"missing simulator {SIM}; run make WIDTH=1792 HEIGHT=64 all first")
    artifact_root = Path(os.environ.get("AV1_ARTIFACT_ROOT", TB / "artifacts"))
    out = artifact_root / "natural1792x64_ip_fullcoeff_newmv"
    ...
    if globalmv != 1537 or newmv != 100 or nearestmv != 155 or nearmv != 0:
        fail(
            f"expected width-stretched unrestricted mix GLOBALMV=1537 NEWMV=100 NEARESTMV=155 NEARMV=0, "
            f"saw global={globalmv} new={newmv} nearest={nearestmv} near={nearmv}"
        )
    print(
        "[PASS] 1792x64 full-coeff unrestricted integer motion summary: "
    ...
    check_public_decoder_case(paths, "1792x64 full-coeff unrestricted integer motion")
    print("[PASS] 1792x64 full-coeff unrestricted integer motion public-decoder proof")
    return 0
