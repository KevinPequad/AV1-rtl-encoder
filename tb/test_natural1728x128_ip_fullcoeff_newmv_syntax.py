#!/usr/bin/env python3
"""1728x128 unrestricted full-coeff low-delay LAST inter proof.

This graduates the next wider 128-high full-coeff NEWMV/reference-stack
checkpoint beyond 1664x128. It proves the deterministic 1728x128 gradient stress
...
def main() -> int:
    if not SIM.exists():
        fail(f"missing simulator {SIM}; run make WIDTH=1728 HEIGHT=128 all first")
    artifact_root = Path(os.environ.get("AV1_ARTIFACT_ROOT", TB / "artifacts"))
    out = artifact_root / "natural1728x128_ip_fullcoeff_newmv"
    ...
    if globalmv != 3201 or newmv != 100 or nearestmv != 155 or nearmv != 0:
        fail(
            f"expected unrestricted taller-geometry mix GLOBALMV=3201 NEWMV=100 NEARESTMV=155 NEARMV=0, "
            f"saw global={globalmv} new={newmv} nearest={nearestmv} near={nearmv}"
        )
    print(
        "[PASS] 1728x128 unrestricted full-coeff integer motion summary: "
    ...
    check_public_decoder_case(paths, "1728x128 unrestricted full-coeff integer motion")
    print("[PASS] 1728x128 unrestricted full-coeff integer motion public-decoder proof")
    return 0
