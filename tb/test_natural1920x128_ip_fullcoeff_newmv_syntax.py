#!/usr/bin/env python3
"""1920x128 unrestricted full-coeff low-delay LAST inter proof.

This graduates the next wider 128-high full-coeff NEWMV/reference-stack
checkpoint beyond 1856x128. It proves the deterministic 1920x128 gradient stress
...
def main() -> int:
    ...
    out = artifact_root / "natural1920x128_ip_fullcoeff_newmv"
    ...
    if globalmv != 3796 or newmv != 42 or nearestmv != 2 or nearmv != 0:
        fail(
            f"expected unrestricted taller-geometry mix GLOBALMV=3796 NEWMV=42 NEARESTMV=2 NEARMV=0, "
            f"saw global={globalmv} new={newmv} nearest={nearestmv} near={nearmv}"
        )
    print(
        "[PASS] 1920x128 unrestricted full-coeff integer motion summary: "
    ...
    check_public_decoder_case(paths, "1920x128 unrestricted full-coeff integer motion")
    print("[PASS] 1920x128 unrestricted full-coeff integer motion public-decoder proof")
    return 0
