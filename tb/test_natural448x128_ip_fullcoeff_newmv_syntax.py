#!/usr/bin/env python3
"""448x128 unrestricted full-coeff low-delay LAST inter proof.

This graduates the next wider 128-high full-coeff NEWMV/reference-stack
checkpoint beyond 384x128. It proves the deterministic 448x128 gradient stress
...
def main() -> int:
    if not SIM.exists():
        fail(f"missing simulator {SIM}; run make WIDTH=448 HEIGHT=128 all first")
    artifact_root = Path(os.environ.get("AV1_ARTIFACT_ROOT", TB / "artifacts"))
    out = artifact_root / "natural448x128_ip_fullcoeff_newmv"
    ...
    if globalmv != 852 or newmv != 42 or nearestmv != 2 or nearmv != 0:
        fail(
            f"expected unrestricted taller-geometry mix GLOBALMV=852 NEWMV=42 NEARESTMV=2 NEARMV=0, "
            f"saw global={globalmv} new={newmv} nearest={nearestmv} near={nearmv}"
        )
    print(
        "[PASS] 448x128 unrestricted full-coeff integer motion summary: "
    ...
    check_public_decoder_case(paths, "448x128 unrestricted full-coeff integer motion")
    print("[PASS] 448x128 unrestricted full-coeff integer motion public-decoder proof")
    return 0
