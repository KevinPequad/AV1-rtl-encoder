#!/usr/bin/env python3
"""192x192 full-coeff low-delay LAST inter proof with unrestricted integer motion.

This extends the taller-than-128 NEWMV/reference-stack public-clean lane to the
192x192 natural gradient stress path. It proves this square multi-superblock
geometry is RTL/software byte-identical and public-decoder-to-recon exact
without treating cap growth as feature-complete closure.
"""
...
def main() -> int:
    if not SIM.exists():
        fail(f"missing simulator {SIM}; run make WIDTH=192 HEIGHT=192 all first")
    artifact_root = Path(os.environ.get("AV1_ARTIFACT_ROOT", TB / "artifacts"))
    out = artifact_root / "natural192x192_ip_fullcoeff_newmv"
    ...
    if globalmv != 532 or newmv != 42 or nearestmv != 2 or nearmv != 0:
        fail(
            f"expected 192x192 unrestricted mix GLOBALMV=532 NEWMV=42 NEARESTMV=2 NEARMV=0, "
            f"saw global={globalmv} new={newmv} nearest={nearestmv} near={nearmv}"
        )
    print(
        "[PASS] 192x192 full-coeff unrestricted integer motion summary: "
    ...
    check_public_decoder_case(paths, "192x192 full-coeff unrestricted integer motion")
    print("[PASS] 192x192 full-coeff unrestricted integer motion public-decoder proof")
    return 0
