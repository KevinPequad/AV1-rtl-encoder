#!/usr/bin/env python3
"""1920x128 explicit cap-disable full-coeff low-delay LAST proof.

This pins the uncapped behavior for the next 128-high full-coeff
NEWMV/reference-stack checkpoint beyond 1856x128. It intentionally uses the
testbench-only +disable_fullcoeff_cap=1 probe plus +me_newmv_limit=255 so future
cap-hidden passes cannot masquerade as unrestricted 1920x128 behavior.
"""
...
def main() -> int:
    if not SIM.exists():
        fail(f"missing simulator {SIM}; run make WIDTH=1920 HEIGHT=128 all first")
    artifact_root = Path(os.environ.get("AV1_ARTIFACT_ROOT", TB / "artifacts"))
    out = artifact_root / "natural1920x128_ip_fullcoeff_newmv_uncapped_probe"
    ...
    if globalmv != 3585 or newmv != 100 or nearestmv != 155 or nearmv != 0:
        fail(
            f"expected uncapped cap-disable mix GLOBALMV=3585 NEWMV=100 NEARESTMV=155 NEARMV=0, "
            f"saw global={globalmv} new={newmv} nearest={nearestmv} near={nearmv}"
        )
    print(
        "[PASS] 1920x128 cap-disable full-coeff integer motion summary: "
    ...
    check_public_decoder_case(paths, "1920x128 cap-disable full-coeff integer motion")
    print("[PASS] 1920x128 cap-disable full-coeff integer motion public-decoder proof")
    return 0
