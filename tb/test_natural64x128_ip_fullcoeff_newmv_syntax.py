#!/usr/bin/env python3
"""64x128 full-coeff low-delay LAST inter proof with unrestricted integer motion.

This extends the taller-than-64 NEWMV/reference-stack public-clean lane to the
64x128 natural gradient stress path. It keeps the broader taller-geometry cap
...
def main() -> int:
    if not SIM.exists():
        fail(f"missing simulator {SIM}; run make WIDTH=64 HEIGHT=128 all first")
    artifact_root = Path(os.environ.get("AV1_ARTIFACT_ROOT", TB / "artifacts"))
    out = artifact_root / "natural64x128_ip_fullcoeff_newmv"
    ...
    if globalmv != 58 or newmv != 44 or nearestmv != 26 or nearmv != 0:
        fail(
            f"expected 64x128 unrestricted mix GLOBALMV=58 NEWMV=44 NEARESTMV=26 NEARMV=0, "
            f"saw global={globalmv} new={newmv} nearest={nearestmv} near={nearmv}"
        )
    print(
        "[PASS] 64x128 full-coeff unrestricted integer motion summary: "
    ...
    check_public_decoder_case(paths, "64x128 full-coeff unrestricted integer motion")
    print("[PASS] 64x128 full-coeff unrestricted integer motion public-decoder proof")
    return 0
