#!/usr/bin/env python3
"""1536x192 full-coeff low-delay LAST inter proof with unrestricted integer motion.

This extends the taller-than-128 NEWMV/reference-stack public-clean lane beyond
1472x192. It proves the 1536x192 rectangular multi-superblock gradient stress
...
def main() -> int:
    if not SIM.exists():
        fail(f"missing simulator {SIM}; run make WIDTH=1536 HEIGHT=192 all first")
    artifact_root = Path(os.environ.get("AV1_ARTIFACT_ROOT", TB / "artifacts"))
    out = artifact_root / "natural1536x192_ip_fullcoeff_newmv"
    ...
    if globalmv != 4564 or newmv != 42 or nearestmv != 2 or nearmv != 0:
        fail(
            f"expected 1536x192 unrestricted mix GLOBALMV=4564 NEWMV=42 NEARESTMV=2 NEARMV=0, "
            f"saw global={globalmv} new={newmv} nearest={nearestmv} near={nearmv}"
        )
    print(
        "[PASS] 1536x192 full-coeff unrestricted integer motion summary: "
    ...
    check_public_decoder_case(paths, "1536x192 full-coeff unrestricted integer motion")
    print("[PASS] 1536x192 full-coeff unrestricted integer motion public-decoder proof")
    return 0
