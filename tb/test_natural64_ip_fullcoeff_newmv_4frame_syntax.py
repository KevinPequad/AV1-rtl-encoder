#!/usr/bin/env python3
"""4-frame 64x64 full-coeff low-delay LAST unrestricted NEWMV proof.

This widens the strict 3-frame full-coeff NEWMV public-decoder/recon parity
checkpoint by one more LAST refresh.  It keeps the unrestricted reduced
reference-stack path honest without raising the NEWMV cap: frame 3 stays
public-decoder exact while settling to the current 18 NEWMV / 2 NEARESTMV mix.
"""
...
def main() -> int:
    ...
    out = artifact_root / "natural64_ip_fullcoeff_newmv_4frame"
    ...
    check_public_decoder_case(paths, "4-frame 64x64 full-coeff unrestricted NEWMV LAST-chain")
    ...
    for label, path in (
        ...
        if got != expected_md5s:
            fail(f"{label} 4-frame MD5 scope drifted: {got}")

    print(
        "[PASS] 4-frame 64x64 full-coeff unrestricted NEWMV LAST-chain: "
        "RTL/software bytes match, FFmpeg/libdav1d and aomdec decode cleanly, "
        "decoder output is byte-identical to RTL recon, and frame-3 stays at "
        "GLOBALMV=44 NEARESTMV=2 NEARMV=0 NEWMV=18"
    )
    return 0
