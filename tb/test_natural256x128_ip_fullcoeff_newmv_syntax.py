#!/usr/bin/env python3
"""256x128 unrestricted full-coeff low-delay LAST inter proof.
...
deterministic 256x128 gradient stress path is RTL/software byte-identical and
...
def main() -> int:
    if not SIM.exists():
        fail(f"missing simulator {SIM}; run make WIDTH=256 HEIGHT=128 all first")
    artifact_root = Path(os.environ.get("AV1_ARTIFACT_ROOT", TB / "artifacts"))
    out = artifact_root / "natural256x128_ip_fullcoeff_newmv"
    ...
    if globalmv != 394 or newmv != 61 or nearestmv != 57 or nearmv != 0:
        fail(
            f"expected unrestricted taller-geometry mix GLOBALMV=394 NEWMV=61 NEARESTMV=57 NEARMV=0, "
            f"saw global={globalmv} new={newmv} nearest={nearestmv} near={nearmv}"
        )
    print(
        "[PASS] 256x128 unrestricted full-coeff integer motion summary: "
    ...
    check_public_decoder_case(paths, "256x128 unrestricted full-coeff integer motion")
    print("[PASS] 256x128 unrestricted full-coeff integer motion public-decoder proof")
    return 0
