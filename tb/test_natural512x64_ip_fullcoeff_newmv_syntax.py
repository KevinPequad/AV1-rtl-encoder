#!/usr/bin/env python3
"""512x64 full-coeff low-delay LAST inter proof with unrestricted integer motion.

This extends the width-stretched 64-high NEWMV/reference-stack public-clean
lane beyond 448x64. It keeps the taller-than-64 cap intact while proving the
deterministic 512x64 gradient stress path is RTL/software byte-identical and
...
def main() -> int:
    if not SIM.exists():
        fail(f"missing simulator {SIM}; run make WIDTH=512 HEIGHT=64 all first")
    artifact_root = Path(os.environ.get("AV1_ARTIFACT_ROOT", TB / "artifacts"))
    out = artifact_root / "natural512x64_ip_fullcoeff_newmv"
    ...
    if globalmv != 354 or newmv != 76 or nearestmv != 82 or nearmv != 0:
        fail(
            f"expected width-stretched unrestricted mix GLOBALMV=354 NEWMV=76 NEARESTMV=82 NEARMV=0, "
            f"saw global={globalmv} new={newmv} nearest={nearestmv} near={nearmv}"
        )
    print(
        "[PASS] 512x64 full-coeff unrestricted integer motion summary: "
    ...
    check_public_decoder_case(paths, "512x64 full-coeff unrestricted integer motion")
    print("[PASS] 512x64 full-coeff unrestricted integer motion public-decoder proof")
    return 0
