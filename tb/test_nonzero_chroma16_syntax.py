...
def cmp_file(a: Path, b: Path, label: str):
    if a.read_bytes() != b.read_bytes(): raise SystemExit(f"[FAIL] {label}: {a} != {b}")
    print(f"[PASS] {label}")
require("ffmpeg"); require("aomdec")
if not SIM.exists(): raise SystemExit(f"missing simulator {SIM}; run make WIDTH=16 HEIGHT=16 all first")
