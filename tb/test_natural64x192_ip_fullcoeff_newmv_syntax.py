#!/usr/bin/env python3
"""64x192 full-coeff low-delay LAST inter proof with unrestricted integer motion.

This extends the taller-than-64 NEWMV/reference-stack public-clean lane to the
64x192 natural gradient stress path. It keeps the broader taller-geometry cap
intact while proving this exact path is RTL/software byte-identical and
public-decoder-to-recon exact.
"""
...
def main() -> int:
    if not SIM.exists():
        fail(f"missing simulator {SIM}; run make WIDTH=64 HEIGHT=192 all first")
    artifact_root = Path(os.environ.get("AV1_ARTIFACT_ROOT", TB / "artifacts"))
    out = artifact_root / "natural64x192_ip_fullcoeff_newmv"
    ...
    if globalmv != 148 or newmv != 42 or nearestmv != 2 or nearmv != 0:
        fail(
            f"expected 64x192 unrestricted mix GLOBALMV=148 NEWMV=42 NEARESTMV=2 NEARMV=0, "
            f"saw global={globalmv} new={newmv} nearest={nearestmv} near={nearmv}"
        )
    print(
        "[PASS] 64x192 full-coeff unrestricted integer motion summary: "
    ...
    check_public_decoder_case(paths, "64x192 full-coeff unrestricted integer motion")
    print("[PASS] 64x192 full-coeff unrestricted integer motion public-decoder proof")
    return 0
